...
def Loglikelihood(parameters, observations_data, observations_data_sem, drug_application_time=0, total_simulation_time=201, time_points=None):
    parameter_values = {
    ...
    try:
        ...
        if time_points is not None:
            ...
            for i in range(len(observations_data[0])):  # Assuming observations[0] is correctly indexed
                loglikelihood -= (np.sum(observations_data[0][i]) - y_P[i])**2 / (2 * observations_data_sem[0][i]**2)
                loglikelihood -= (np.sum(observations_data[1][i]) - y_R_n[i])**2 / (2 * observations_data_sem[1][i]**2)
                loglikelihood -= (np.sum(observations_data[2][i]) - y_R_c[i])**2 / (2 * observations_data_sem[2][i]**2)
    except:
        loglikelihood = -1e20
        time = None
...
def negative_log_posterior(theta_untransformed, observations_data, observations_data_sem, drug_application_time, total_simulation_time, time_points):
    ...
    log_likelihood = Loglikelihood(theta, observations_data, observations_data_sem, drug_application_time, total_simulation_time, time_points)[0]
    return (log_prior + log_likelihood)  # Negative because minimize seeks to minimize the function
# running the MCMC
if run_ms:
    bg_log_target_partial = partial(negative_log_posterior, observations_data=observations_data,observations_data_sem=observations_data_sem, drug_application_time=drug_application_time, total_simulation_time=total_simulation_time, time_points=time_points)
    ...
    bg_chain, bg_logpos = adaptive_metropolis(bg_log_target_partial, start=chain_start, chain_len=chain_length, initial_scale=0.0001, rng=np.random.default_rng())
...
ll, time, concentrations_species = Loglikelihood(best_fit, observations_data=observations_data,observations_data_sem=observations_data_sem,drug_application_time=drug_application_time,total_simulation_time=total_simulation_time, time_points=time_points)
print(ll)
# solving the model with the true parameters
print('true parameters:', true_parameter_values)
ll, time, concentrations_species_true = Loglikelihood(true_parameter_values, observations_data=observations_data,observations_data_sem=observations_data_sem,drug_application_time=drug_application_time,total_simulation_time=total_simulation_time, time_points=time_points)
print(ll)
...
#plt.plot(time_points, observations_data[0], 'o', color=species_colors['P'], label=' Spatial Model (P)', markersize=12)
#plt.plot(time_points, observations_data[1], 'o', color=species_colors['R_n'], label=' Spatial Model (R_n)', markersize=12)
#plt.plot(time_points, observations_data[2], 'o', color=species_colors['R_c'], label=' Spatial Model (R_c)', markersize=12)
# plotting the std for observations_data
plt.errorbar(time_points, observations_data[0], yerr=observations_data_sem[0], fmt='o', color=species_colors['P'], label='Spatial Model ' + f"${'(P)'}$", markersize=10, lw=1)
plt.errorbar(time_points, observations_data[1], yerr=observations_data_sem[1], fmt='o', color=species_colors['R_n'], label='Spatial Model ' + f"${'(R_n)'}$", markersize=10, lw=1)
plt.errorbar(time_points, observations_data[2], yerr=observations_data_sem[2], fmt='o', color=species_colors['R_c'], label= 'Spatial Model ' + f"${'(R_c)'}$", markersize=10, lw=1)
...
# plt.plot(time, concentrations_species_true['P'], color=species_colors['P'], label='SSA + true param (P)', lw=2, linestyle='--')
# plt.plot(time, concentrations_species_true['R_n'], color=species_colors['R_n'], label='SSA + true param (R_n)', lw=2, linestyle='--')
# plt.plot(time, concentrations_species_true['R_c'], color=species_colors['R_c'], label='SSA + true param (R_c)', lw=2, linestyle='--')
