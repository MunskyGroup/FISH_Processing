...
class Utilities():
    ...
    def convert_to_standard_format(self, data_folder_path, path_to_config_file, number_color_channels=2,
                                   number_of_fov=1, download_data_from_NAS=True, use_metadata=False,
                ...
                for j in range(number_of_fov):
                    ...
                    if use_metadata == False:
                        ...
                        if number_z_slices > 50:
                            raise ValueError('The number of automatically detected z slices is ' + str(
                                number_z_slices) + ', double-check the number_of_fov and number_color_channels.')
                ...
                for k in range(number_images_all_fov):
                    ...
                    if detected_metadata == False:
                        if (number_total_images_in_fov % (number_color_channels * number_of_fov)) == 0:
                            ...
                            raise ValueError(
                                'The number of z slices is not defined correctly double-check the number_of_fov and number_color_channels.')
                        if number_z_slices > 50:
                            raise ValueError('The number of automatically detected z slices is ' + str(
                                number_z_slices) + ', double-check the number_of_fov and number_color_channels.')
                    ...
                    for i in range(number_of_fov):
                        list_files_names.append(
                            list_files_names_all_fov[k].split(".")[0] + '_img_' + str(k) + '_fov_' + str(i) + '.tif')
                        temp_image_fov = np.zeros((number_elements_on_fov, y_shape, x_shape))
                        temp_image_fov = image_with_all_fov[
                                         counter * number_elements_on_fov:number_elements_on_fov * (counter + 1), :, :]
                        temp_image = np.zeros((number_z_slices, y_shape, x_shape, number_color_channels))
                        for ch in range(number_color_channels):
                            temp_image[:, :, :, ch] = temp_image_fov[ch::number_color_channels, :, :]
    ...
    def create_output_folders(self, data_folder_path, diameter_nucleus, diameter_cytosol, psf_z, psf_yx,
                              threshold_for_spot_detection, channels_with_FISH, list_threshold_for_spot_detection):
        ...
        if (threshold_for_spot_detection is None):
            output_identification_string = original_folder_name + '___nuc_' + str(diameter_nucleus) + '__cyto_' + str(
                diameter_cytosol) + '__psfz_' + str(psf_z) + '__psfyx_' + str(psf_yx) + '__ts_auto'
        else:
            output_identification_string = original_folder_name + '___nuc_' + str(diameter_nucleus) + '__cyto_' + str(
                diameter_cytosol) + '__psfz_' + str(psf_z) + '__psfyx_' + str(psf_yx) + '__ts'
    ...
    def create_list_thresholds_FISH(self, channels_with_FISH, threshold_for_spot_detection=None):
        ...
        if (isinstance(list_threshold_for_spot_detection, list)) and (
                len(list_threshold_for_spot_detection) < len(channels_with_FISH)):
            for i in range(len(channels_with_FISH)):
        ...
        if not (n_masks is None):
            if n_masks > 1:  # detecting if more than 1 mask are detected per cell
                for nm in range(1,
                    ...
                    tested_mask = np.where(mask_copy == nm, 1,
                                           0)  # making zeros all elements outside each mask, and once all elements inside of each mask.
        ...
        if rescale == True:
            ...
            for ch in range(image.shape[2]):
                if np.max(image[:, :, ch]) > 0:
                    im_zeros[:, :, ch] = RemoveExtrema(image[:, :, ch], min_percentile=min_percentile,
                                                       max_percentile=max_percentile).remove_outliers()
            image = im_zeros
    ...
    def read_zipfiles_from_NAS(self, list_dirs, path_to_config_file, share_name, mandatory_substring,
                               local_folder_path):
        ...
        for folder in list_dirs:
            ...
            for file in list_files:
                if ('.zip' in file) and (mandatory_substring in file):  # add an argument with re conditions
                    ...
                    NASConnection(path_to_config_file, share_name=share_name).download_file(zip_file_path,
                                                                                            local_folder_path,
                                                                                            timeout=200)
    ...
    def dataframe_extract_data(self, dataframe, spot_type, minimum_spots_cluster=2):
        ...
        number_of_spots_per_cell_cytosol = np.asarray([len(dataframe.loc[(dataframe['cell_id'] == i) & (
                    dataframe['is_nuc'] == False) & (dataframe['spot_type'] == spot_type) & (dataframe[
                                                                                                 'is_cell_fragmented'] != -1)].spot_id)
                                                       for i in range(0, number_cells)])
        # Number of spots in nucleus.  Spots without TS.
        number_of_spots_per_cell_nucleus = np.asarray([len(dataframe.loc[(dataframe['cell_id'] == i) & (
                    dataframe['is_nuc'] == True) & (dataframe['spot_type'] == spot_type) & (dataframe[
                                                                                                'is_cell_fragmented'] != -1)].spot_id)
                                                       for i in range(0, number_cells)])
        # Number of spots
        number_of_spots_per_cell = np.asarray([len(dataframe.loc[(dataframe['cell_id'] == i) & (
                    dataframe['spot_type'] == spot_type) & (dataframe['is_cell_fragmented'] != -1)].spot_id) for i in
                                               range(0, number_cells)])
        # Number of TS per cell.
        number_of_TS_per_cell = [len(dataframe.loc[(dataframe['cell_id'] == i) & (dataframe['is_cluster'] == True) & (
                    dataframe['is_nuc'] == True) & (dataframe['spot_type'] == spot_type) & (
                                                               dataframe['cluster_size'] >= minimum_spots_cluster) & (
                                                               dataframe['is_cell_fragmented'] != -1)].spot_id) for i in
                                 range(0, number_cells)]
        number_of_TS_per_cell = np.asarray(number_of_TS_per_cell)
        # Number of RNA in a TS
        ts_size = dataframe.loc[(dataframe['is_cluster'] == True) & (dataframe['is_nuc'] == True) & (
                    dataframe['spot_type'] == spot_type) & (dataframe['cluster_size'] >= minimum_spots_cluster) & (
                                            dataframe['is_cell_fragmented'] != -1)].cluster_size.values
        # Size of each cell
    ...
    def extracting_data_for_each_df_in_directory(self, list_local_folders, current_dir, spot_type=0,
        ...
        for i in range(0, len(list_local_folders)):
            dataframe_dir = current_dir.joinpath('analyses',
                                                 list_local_folders[i])  # loading files from "analyses" folder
            ...
            number_of_spots_per_cell, number_of_spots_per_cell_cytosol, number_of_spots_per_cell_nucleus, number_of_TS_per_cell, ts_size, cell_size, number_cells, nuc_size, cyto_size = Utilities().dataframe_extract_data(
                dataframe, spot_type, minimum_spots_cluster=minimum_spots_cluster)
    ...
    def extract_data_interpretation(self, list_dirs, path_to_config_file, current_dir, mandatory_substring,
                                    local_folder_path, list_labels, share_name='share', minimum_spots_cluster=2,
                                    connect_to_NAS=0, spot_type=0, remove_extreme_values=False):
        if connect_to_NAS == True:
            # Reading the data from NAS, unziping files, organizing data as single dataframe for comparison.
            list_local_files = Utilities().read_zipfiles_from_NAS(list_dirs, path_to_config_file, share_name,
                                                                  mandatory_substring, local_folder_path)
        ...
        list_spots_total, list_spots_nuc, list_spots_cytosol, list_number_cells, list_transcription_sites, list_cell_size, list_dataframes, list_nuc_size, list_cyto_size = Utilities().extracting_data_for_each_df_in_directory(
            list_local_folders=list_local_folders, current_dir=current_dir, spot_type=spot_type,
            minimum_spots_cluster=minimum_spots_cluster)
        # Final dataframes for nuc, cyto and total spots
        df_all = Utilities().convert_list_to_df(list_number_cells, list_spots_total, list_labels,
                                                remove_extreme_values=remove_extreme_values)
        df_cyto = Utilities().convert_list_to_df(list_number_cells, list_spots_cytosol, list_labels,
                                                 remove_extreme_values=remove_extreme_values)
        df_nuc = Utilities().convert_list_to_df(list_number_cells, list_spots_nuc, list_labels,
                                                remove_extreme_values=remove_extreme_values)
        df_transcription_sites = Utilities().convert_list_to_df(list_number_cells, list_transcription_sites,
                                                                list_labels,
                                                                remove_extreme_values=remove_extreme_values)
    ...
    def function_get_df_columns_as_array(self, df, colum_to_extract, extraction_type='all_values'):
        ...
        if extraction_type == 'values_per_cell':
            return np.asarray(
                [df.loc[(df['cell_id'] == i)][colum_to_extract].values[0] for i in range(0, number_cells)])
    ...
    def convert_list_to_df(self, list_number_cells, list_spots, list_labels, remove_extreme_values=False,
                           max_quantile=0.98):
    ...
    def download_data_NAS(self, path_to_config_file, data_folder_path, path_to_masks_dir, share_name, timeout=200):
        ...
        NASConnection(path_to_config_file, share_name=share_name).copy_files(data_folder_path, local_folder_path,
                                                                             timeout=timeout)
        ...
        if not (path_to_masks_dir is None):
            ...
            NASConnection(path_to_config_file, share_name=share_name).download_file(path_to_masks_dir,
                                                                                    local_folder_path_masks,
                                                                                    timeout=timeout)
    ...
    def read_images_from_folder(self, path_to_config_file, data_folder_path, path_to_masks_dir=None,
                                download_data_from_NAS=False, substring_to_detect_in_file_name='.*_C0.tif'):
        # Download data from NAS
        if download_data_from_NAS == True:
            share_name = 'share'
            local_data_dir, masks_dir = Utilities().download_data_NAS(path_to_config_file, data_folder_path,
                                                                      path_to_masks_dir, share_name, timeout=200)
        else:
            local_data_dir = data_folder_path
            masks_dir = path_to_masks_dir
            # Detecting if images need to be merged
        is_needed_to_merge_images = MergeChannels(local_data_dir,
                                                  substring_to_detect_in_file_name=substring_to_detect_in_file_name,
                                                  save_figure=1).checking_images()
        if is_needed_to_merge_images == True:
            _, _, number_images, _ = MergeChannels(local_data_dir,
                                                   substring_to_detect_in_file_name=substring_to_detect_in_file_name,
                                                   save_figure=1).merge()
            ...
            list_images, path_files, list_files_names, number_images = ReadImages(
                directory=local_data_dir).read()  # list_images, path_files, list_files_names, number_files
        ...
        if not (list_files_distributions is None) and (type(list_files_distributions) is list):
            ...
            for i in range(len(file_plots_distributions)):
                if not (file_plots_distributions is None):
                    pathlib.Path().absolute().joinpath(file_plots_distributions[i]).rename(
                        pathlib.Path().absolute().joinpath(str('analysis_' + output_identification_string),
                                                           file_plots_distributions[i]))
                if not (file_plots_cell_size_vs_num_spots is None):
                    pathlib.Path().absolute().joinpath(file_plots_cell_size_vs_num_spots[i]).rename(
                        pathlib.Path().absolute().joinpath(str('analysis_' + output_identification_string),
                                                           file_plots_cell_size_vs_num_spots[i]))
                if not (file_plots_cell_intensity_vs_num_spots is None):
                    pathlib.Path().absolute().joinpath(file_plots_cell_intensity_vs_num_spots[i]).rename(
                        pathlib.Path().absolute().joinpath(str('analysis_' + output_identification_string),
                                                           file_plots_cell_intensity_vs_num_spots[i]))
                if not (file_plots_spot_intensity_distributions is None):
                    pathlib.Path().absolute().joinpath(file_plots_spot_intensity_distributions[i]).rename(
                        pathlib.Path().absolute().joinpath(str('analysis_' + output_identification_string),
                                                           file_plots_spot_intensity_distributions[i]))

        if not (file_plots_bleed_thru is None):
            pathlib.Path().absolute().joinpath(file_plots_bleed_thru).rename(
                pathlib.Path().absolute().joinpath(str('analysis_' + output_identification_string),
                                                   file_plots_bleed_thru))
        if not (file_plots_int_ratio is None):
            pathlib.Path().absolute().joinpath(file_plots_int_ratio).rename(
                pathlib.Path().absolute().joinpath(str('analysis_' + output_identification_string),
                                                   file_plots_int_ratio))
        if not (file_plots_int_pseudo_ratio is None):
            pathlib.Path().absolute().joinpath(file_plots_int_pseudo_ratio).rename(
                pathlib.Path().absolute().joinpath(str('analysis_' + output_identification_string),
                                                   file_plots_int_pseudo_ratio))

        # all original images
        pathlib.Path().absolute().joinpath('original_images_' + data_folder_path.name + '.pdf').rename(
            pathlib.Path().absolute().joinpath(str('analysis_' + output_identification_string),
                                               'original_images_' + data_folder_path.name + '.pdf'))
        # all cell images
        for i in range(len(channels_with_FISH)):
            temp_plot_name = 'cells_channel_' + str(channels_with_FISH[i]) + '_' + data_folder_path.name + '.pdf'
            pathlib.Path().absolute().joinpath(temp_plot_name).rename(
                pathlib.Path().absolute().joinpath(str('analysis_' + output_identification_string), temp_plot_name))
        # metadata_path
        pathlib.Path().absolute().joinpath('images_report_' + data_folder_path.name + '.csv').rename(
            pathlib.Path().absolute().joinpath(str('analysis_' + output_identification_string),
                                               'images_report_' + data_folder_path.name + '.csv'))
        pathlib.Path().absolute().joinpath('metadata_' + data_folder_path.name + '.txt').rename(
            pathlib.Path().absolute().joinpath(str('analysis_' + output_identification_string),
                                               'metadata_' + data_folder_path.name + '.txt'))
        # dataframe_path
        pathlib.Path().absolute().joinpath('dataframe_' + data_folder_path.name + '.csv').rename(
            pathlib.Path().absolute().joinpath(str('analysis_' + output_identification_string),
                                               'dataframe_' + data_folder_path.name + '.csv'))
        # pdf_path
        if save_pdf_report == True:
            pathlib.Path().absolute().joinpath('pdf_report_' + data_folder_path.name + '.pdf').rename(
                pathlib.Path().absolute().joinpath(str('analysis_' + output_identification_string),
                                                   'pdf_report_' + data_folder_path.name + '.pdf'))
        # pdf_path segmentation
        pathlib.Path().absolute().joinpath('segmentation_images_' + data_folder_path.name + '.pdf').rename(
            pathlib.Path().absolute().joinpath(str('analysis_' + output_identification_string),
                                               'segmentation_images_' + data_folder_path.name + '.pdf'))

        return None

    def sending_data_to_NAS(self, output_identification_string, data_folder_path, path_to_config_file,
                            path_to_masks_dir, diameter_nucleus, diameter_cytosol, send_data_to_NAS=False,
                            masks_dir=None, share_name='share'):
        ...
        if send_data_to_NAS == True:
            ...
            NASConnection(path_to_config_file, share_name=share_name).write_files_to_NAS(local_file_to_send_to_NAS,
                                                                                         data_folder_path)
        ...
        if path_to_masks_dir == None:
            ...
            name_final_masks = data_folder_path.name + '___nuc_' + str(diameter_nucleus) + '__cyto_' + str(
                diameter_cytosol)
        ...
        if (send_data_to_NAS == True) and (path_to_masks_dir == None):
            shutil.make_archive(mask_dir_complete_name, 'zip',
                                pathlib.Path().absolute().joinpath(mask_dir_complete_name))
    ...
    def move_results_to_analyses_folder(self, output_identification_string, data_folder_path, mask_dir_complete_name,
                                        path_to_masks_dir, save_filtered_images=False, download_data_from_NAS=False):
        ...
        if save_filtered_images == True:
            ...
            pathlib.Path().absolute().joinpath(filtered_folder_name).rename(
                pathlib.Path().absolute().joinpath('analyses', str('analysis_' + output_identification_string),
                                                   filtered_folder_name))
    ...
    def export_data_to_CSV(self, list_spots_total, list_spots_nuc, list_spots_cytosol, destination_folder,
                           plot_title_suffix=''):
    ...
    def extract_images_masks_dataframe(self, data_folder_path, mandatory_substring, path_to_config_file, connect_to_NAS,
                                       path_to_masks_dir=None, rescale=False, max_percentile=99.5):
        ...
        if connect_to_NAS == True:
            list_local_files = Utilities().read_zipfiles_from_NAS(list_dirs=data_folder_path,
        ...
        local_data_dir, masks_dir, number_images, number_color_channels, list_files_names, _ = Utilities().read_images_from_folder(
            path_to_config_file, data_folder_path=data_folder_path, path_to_masks_dir=path_to_masks_dir,
            download_data_from_NAS=connect_to_NAS, substring_to_detect_in_file_name='.*_C0.tif')
    ...
    def image_cell_selection(self, cell_id, list_images, dataframe, mask_cell=None, mask_nuc=None,
                             scaling_value_radius_cell=1.1):
        ...
        subsection_image_with_selected_cell = list_images[selected_image_id][:, y_min_value: y_max_value,
        ...
        if not (mask_cell is None):
            ...
            subsection_mask_cell[0, :] = 0;
            subsection_mask_cell[-1, :] = 0;
            subsection_mask_cell[:, 0] = 0;
            subsection_mask_cell[:, -1] = 0
        else:
        ...
        if not (mask_nuc is None):
            ...
            subsection_mask_nuc[0, :] = 0;
            subsection_mask_nuc[-1, :] = 0;
            subsection_mask_nuc[:, 0] = 0;
            subsection_mask_nuc[:, -1] = 0
        else:
    ...
    def extract_spot_location_from_cell(self, df, spot_type=0, min_ts_size=None, z_slice=None):
        ...
        if not (min_ts_size is None):
            df_TS = df.loc[
                (df['spot_type'] == spot_type) & (df['is_cluster'] == 1) & (df['cluster_size'] >= min_ts_size)]
        else:
    ...
    def spot_crops(self, image, df, number_crops_to_show, spot_size=5):
        ...
        def return_crop(image, x, y, spot_size):
            ...
            crop_image = image[y + spot_range[0]:y + (spot_range[-1] + 1),
                         x + spot_range[0]:x + (spot_range[-1] + 1)].copy()
            return crop_image
...
class Plots():
    ...
    def plotting_segmentation_images(self, directory, list_files_names, list_segmentation_successful=[None],
                                     image_name='temp.pdf', show_plots=True):
    ...
    def plotting_all_original_images(self, list_images, list_files_names, image_name, show_plots=True):
        ...
        for i in range(0, number_images):
            ...
            img_title = list_files_names[i]
            image_int8 = Utilities().convert_to_int8(max_image[:, :, 0:max_nun_channels], rescale=True,
                                                     min_percentile=1, max_percentile=95)
        ...
        for i in range(0, number_channels):
            if number_z_slices > 1:
                if use_maximum_projection == True:
                    ...
                    rescaled_image = RemoveExtrema(image[center_slice, :, :, i], min_percentile=1,
                                                   max_percentile=98).remove_outliers()
            else:
                rescaled_image = RemoveExtrema(image[center_slice, :, :, i], min_percentile=1,
                                               max_percentile=98).remove_outliers()  # image
    ...
    def plotting_masks_and_original_image(self, image, masks_complete_cells, masks_nuclei, channels_with_cytosol,
                                          channels_with_nucleus, image_name, show_plots, df_labels=None):
        ...
        def erode_mask(img, px_to_remove=1):
            img[0:px_to_remove, :] = 0;
            img[:, 0:px_to_remove] = 0;
            img[img.shape[0] - px_to_remove:img.shape[0] - 1, :] = 0;
            img[:,
            img.shape[1] - px_to_remove: img.shape[1] - 1] = 0  # This line of code ensures that the corners are zeros.
        ...
        im = Utilities().convert_to_int8(max_image[:, :, 0:n_channels], rescale=True, min_percentile=1,
                                         max_percentile=95)
        if np.max(masks_complete_cells) != 0 and not (channels_with_cytosol in (None, [None])) and not (
                channels_with_nucleus in (None, [None])):
            ...
            for i in range(1, n_masks + 1):
                ...
                temp_nucleus_mask[0, :] = 0;
                temp_nucleus_mask[-1, :] = 0;
                temp_nucleus_mask[:, 0] = 0;
                temp_nucleus_mask[:, -1] = 0
                temp_complete_mask[0, :] = 0;
                temp_complete_mask[-1, :] = 0;
                temp_complete_mask[:, 0] = 0;
                temp_complete_mask[:, -1] = 0
                ...
                axes[3].fill(contour_n[:, 1], contour_n[:, 0], facecolor='none', edgecolor='red',
                             linewidth=2)  # mask nucleus
                axes[3].fill(contour_c[:, 1], contour_c[:, 0], facecolor='none', edgecolor='red',
                             linewidth=2)  # mask cytosol
            ...
            if not (channels_with_cytosol in (None, [None])) and (channels_with_nucleus in (None, [None])):
                ...
                for i in range(1, n_masks + 1):
                    ...
                    try:
                        ...
                        if contour_c.shape[0] > NUM_POINTS_MASK_EDGE_LINE:
                            contour_c = signal.resample(contour_c, num=NUM_POINTS_MASK_EDGE_LINE)
                            axes[2].fill(contour_c[:, 1], contour_c[:, 0], facecolor='none', edgecolor='red',
                                         linewidth=2)  # mask cytosol
                    except:
                        contour_c = 0
                ...
                if not (df_labels is None):
                    ...
                    for _, label in enumerate(cell_ids_labels):
                        ...
                        axes[2].text(x=X_cell_location, y=Y_cell_location, s=cell_idx_string, fontsize=12,
                                     color='black')
            if (channels_with_cytosol in (None, [None])) and not (channels_with_nucleus in (None, [None])):
                ...
                for i in range(1, n_masks + 1):
                    ...
                    if contour_n.shape[0] > NUM_POINTS_MASK_EDGE_LINE:
                        contour_n = signal.resample(contour_n, num=NUM_POINTS_MASK_EDGE_LINE)
                    axes[2].fill(contour_n[:, 1], contour_n[:, 0], facecolor='none', edgecolor='red',
                                 linewidth=2)  # mask nucleus
                    axes[2].set(title='Original + Masks')
                if not (df_labels is None):
                    cell_ids_labels = df_labels.loc[:, 'cell_id'].values
    ...
    def dist_plots(self, df, plot_title, destination_folder, y_lim_values=None):
        ...
        plt.show()
        pathlib.Path().absolute().joinpath(name_plot).rename(
            pathlib.Path().absolute().joinpath(destination_folder, name_plot))
        ...
        sns.set(font_scale=1.5)
        sns.set_style("white")
        fig, axes = joypy.joyplot(df, x_range=[-5, pct], bins=25, hist=False, overlap=0.8, linewidth=1, figsize=(7, 5),
                                  colormap=cm.CMRmap)  #
    ...
    def plot_TS_bar_stacked(self, df_original, plot_title, destination_folder, minimum_spots_cluster,
                            remove_zeros=False, normalize=True):
        if (normalize == True) and (remove_zeros == True):
            warnings.warn(
                "Warining: notice that normalization is only possible if zeros are not removed. To normalize the output use the options as follows: remove_zeros=False, normalize=True ")
        df = df_original.copy()
        color_palete = 'OrRd'
        sns.set(font_scale=1.5)
        sns.set_style("white")
    ...
    def plotting_results_as_distributions(self, number_of_spots_per_cell, number_of_spots_per_cell_cytosol,
                                          number_of_spots_per_cell_nucleus, ts_size, number_of_TS_per_cell,
                                          minimum_spots_cluster, numBins=20, output_identification_string=None,
                                          spot_type=0):
        ...
        def plot_probability_distribution(data_to_plot, numBins=10, title='', xlab='', ylab='', color='r',
                                          subplots=False, show_grid=True, fig=plt.figure()):
            n, bins, _ = plt.hist(data_to_plot, bins=numBins, density=False, color=color)
            plt.xlabel(xlab, size=16)
            plt.ylabel(ylab, size=16)
            plt.grid(show_grid)
            plt.text(bins[(len(bins) // 2)], (np.amax(n) // 2).astype(int),
                     'mean = ' + str(round(np.mean(data_to_plot), 1)), fontsize=14, bbox=dict(facecolor='w', alpha=0.5))
            plt.title(title, size=16)
            return (f)

        # Section that generates each subplot
        number_subplots = int(np.any(number_of_spots_per_cell)) + int(np.any(number_of_spots_per_cell_cytosol)) + int(
            np.any(number_of_spots_per_cell_nucleus)) + int(np.any(ts_size)) + int(np.any(number_of_TS_per_cell))
        ...
        if np.any(number_of_spots_per_cell):
            ...
            plot_probability_distribution(number_of_spots_per_cell, numBins=20, title='Total Num Spots per cell',
                                          xlab='Number', ylab=ylab, fig=f, color=selected_color)
        if np.any(number_of_spots_per_cell_cytosol):
            subplot_counter += 1
            f.add_subplot(1, number_subplots, subplot_counter)
            plot_probability_distribution(number_of_spots_per_cell_cytosol, numBins=20, title='Num Spots in Cytosol',
                                          xlab='Number', ylab=ylab, fig=f, color=selected_color)
        if np.any(number_of_spots_per_cell_nucleus):
            subplot_counter += 1
            f.add_subplot(1, number_subplots, subplot_counter)
            plot_probability_distribution(number_of_spots_per_cell_nucleus, numBins=20, title='Num Spots in Nucleus',
                                          xlab='Number', ylab=ylab, fig=f, color=selected_color)
        if np.any(ts_size):
            subplot_counter += 1
            f.add_subplot(1, number_subplots, subplot_counter)
            plot_probability_distribution(ts_size, numBins=20, title='Clusters in nucleus', xlab='RNA per Cluster',
                                          ylab=ylab, fig=f, color=selected_color)
        if np.any(number_of_TS_per_cell):
            subplot_counter += 1
            f.add_subplot(1, number_subplots, subplot_counter)
            plot_probability_distribution(number_of_TS_per_cell, numBins=20, title='Number TS per cell',
                                          xlab='[TS (>= ' + str(minimum_spots_cluster) + ' rna)]', ylab=ylab, fig=f,
                                          color=selected_color)
        plt.savefig(file_name, transparent=False, dpi=360, bbox_inches='tight', format='pdf')
        plt.show()
        return file_name

    def plot_scatter_and_distributions(self, x, y, plot_title, x_label_scatter='cell_size',
                                       y_lable_scatter='number_of_spots_per_cell', destination_folder=None,
                                       selected_color='#1C00FE', save_plot=False, temporal_figure=False):
        r, p = stats.pearsonr(x, y)
        df_join_distribution = pd.DataFrame({x_label_scatter: x, y_lable_scatter: y})
        sns.set(font_scale=1.3)
        b = sns.jointplot(data=df_join_distribution, y=y_lable_scatter, x=x_label_scatter, color=selected_color,
                          marginal_kws=dict(bins=40, rug=True))
        b.plot_joint(sns.rugplot, height=0, color=[0.7, 0.7, 0.7], clip_on=True)
        b.plot_joint(sns.kdeplot, color=[0.5, 0.5, 0.5], levels=5)
        b.plot_joint(sns.regplot, scatter_kws={'color': 'orangered', "s": 10, 'marker': 'o'},
                     line_kws={'color': selected_color, 'lw': 2})
        ...
        if temporal_figure == True:
            file_name = 'temp__' + str(np.random.randint(1000, size=1)[
                                           0]) + '__' + name_plot + '.png'  # generating a random name for the temporal plot
        ...
        if not (destination_folder is None) and (save_plot == True) and (temporal_figure == False):
            pathlib.Path().absolute().joinpath(name_plot).rename(
                pathlib.Path().absolute().joinpath(destination_folder, file_name))
        return b.fig, file_name

    def plot_cell_size_spots(self, channels_with_cytosol, channels_with_nucleus, cell_size, number_of_spots_per_cell,
                             cyto_size, number_of_spots_per_cell_cytosol, nuc_size, number_of_spots_per_cell_nucleus,
                             output_identification_string=None, spot_type=0):
        '''
        ...
        if (cyto_exists == True) and (nuc_exists == True):
            ...
            _, fig1_temp_name = Plots().plot_scatter_and_distributions(x, y, title_plot, x_label_scatter='cell_size',
                                                                       y_lable_scatter='number_of_spots_per_cell',
                                                                       temporal_figure=True)
        # Cytosol
        if cyto_exists == True:
            x = cyto_size
            y = number_of_spots_per_cell_cytosol
            title_plot = 'cytosol'
            _, fig2_temp_name = Plots().plot_scatter_and_distributions(x, y, title_plot, x_label_scatter='cyto_size',
                                                                       y_lable_scatter='number_of_spots_per_cyto',
                                                                       temporal_figure=True)
        # Nucleus
        if nuc_exists == True:
            x = nuc_size
            ...
            title_plot = 'nucleus'
            _, fig3_temp_name = Plots().plot_scatter_and_distributions(x, y, title_plot, x_label_scatter='nuc_size',
                                                                       y_lable_scatter='number_of_spots_per_nuc',
                                                                       temporal_figure=True)
        # Plotting
        _, axes = plt.subplots(nrows=1, ncols=3, figsize=(20, 10))
        counter = 0
    ...
    def plot_cell_intensity_spots(self, dataframe, number_of_spots_per_cell_nucleus=None,
                                  number_of_spots_per_cell_cytosol=None, output_identification_string=None,
                                  spot_type=0):
        # Creating a name for the plot
        if not (output_identification_string is None):
            index_string = output_identification_string.index('__')
        ...
        for j in range(number_rows):
            for i in range(number_color_channels):
                ...
                if (nucleus_exists == True) and (counter == 0):
                    ...
                    title_plot = 'nucleus'
                    x = Utilities().function_get_df_columns_as_array(df=dataframe,
                                                                     colum_to_extract=column_with_intensity,
                                                                     extraction_type='values_per_cell')
                    y = number_of_spots_per_cell_nucleus
                if ((cyto_exists == True) and (counter == 1)) or (
                        (cyto_exists == True) and (counter == 0) and (number_rows == 1)):
                    ...
                    x = Utilities().function_get_df_columns_as_array(df=dataframe,
                                                                     ...
                                                                     extraction_type='values_per_cell')
                    y = number_of_spots_per_cell_cytosol
                _, fig_temp_name = Plots().plot_scatter_and_distributions(x, y, title_plot,
                                                                          x_label_scatter='Intensity_Ch_' + str(i),
                                                                          y_lable_scatter='number_of_spots',
                                                                          temporal_figure=True)
    ...
    def plot_scatter_bleed_thru(self, dataframe, channels_with_cytosol, channels_with_nucleus,
        ...
        for i in range(len(combinations_channels)):
            ...
            if not channels_with_cytosol in (None, 'None', 'none', ['None'], ['none'], [None]):
                x = Utilities().function_get_df_columns_as_array(df=dataframe, colum_to_extract='cyto_int_ch_' + str(
                    combinations_channels[i][0]), extraction_type='values_per_cell')
                y = Utilities().function_get_df_columns_as_array(df=dataframe, colum_to_extract='cyto_int_ch_' + str(
                    combinations_channels[i][1]), extraction_type='values_per_cell')
            if not channels_with_nucleus in (None, 'None', 'none', ['None'], ['none'], [None]):
                x = Utilities().function_get_df_columns_as_array(df=dataframe, colum_to_extract='nuc_int_ch_' + str(
                    combinations_channels[i][0]), extraction_type='values_per_cell')
                y = Utilities().function_get_df_columns_as_array(df=dataframe, colum_to_extract='nuc_int_ch_' + str(
                    combinations_channels[i][1]), extraction_type='values_per_cell')
            _, fig_temp_name = Plots().plot_scatter_and_distributions(x, y, title_plot,
                                                                      x_label_scatter='intensity_Ch_' + str(
                                                                          combinations_channels[i][0]),
                                                                      y_lable_scatter='intensity_Ch_' + str(
                                                                          combinations_channels[i][1]),
                                                                      temporal_figure=True)
            axis_index.imshow(plt.imread(fig_temp_name))
            axis_index.grid(False)
            axis_index.set_xticks([])
    ...
    def plot_interpretation_distributions(self, df_all, df_cyto, df_nuc, destination_folder, plot_title_suffix='',
                                          y_lim_values_all_spots=None, y_lim_values_cyto=None, y_lim_values_nuc=None):
        if (df_cyto.dropna().any().any() == True) and (
                df_nuc.dropna().any().any() == True):  # removing nans from df and then testing if any element is non zero. If this is true, the plot is generated
    ...
    def plot_spot_intensity_distributions(self, dataframe, output_identification_string=None, remove_outliers=True,
                                          spot_type=0):
        # Creating a name for the plot
        if not (output_identification_string is None):
            index_string = output_identification_string.index('__')
        ...
        for i in range(0, number_color_channels):
            ...
            df_spot_intensity = dataframe.loc[
                (dataframe['is_cluster'] == False) & (dataframe['spot_type'] == spot_type)]
    ...
    def plot_nuc_cyto_int_ratio_distributions(self, dataframe, output_identification_string=None,
                                              plot_for_pseudo_cytosol=False, remove_outliers=True):
        # Creating a name for the plot
        if not (output_identification_string is None):
            index_string = output_identification_string.index('__')
            if index_string >= 0:
        ...
        for i in range(0, number_color_channels):
            ...
            int_ratio = np.asarray([dataframe.loc[(dataframe['cell_id'] == i)][colum_to_extract].values[0] for i in
                                    range(0, number_cells)])
            if remove_outliers == True:
    ...
    def plot_all_distributions(self, dataframe, channels_with_cytosol, channels_with_nucleus, channels_with_FISH,
                               minimum_spots_cluster, output_identification_string):
        ...
        for i in range(number_fish_channels):
            number_of_spots_per_cell, number_of_spots_per_cell_cytosol, number_of_spots_per_cell_nucleus, number_of_TS_per_cell, ts_size, cell_size, number_cells, nuc_size, cyto_size = Utilities().dataframe_extract_data(
                dataframe, spot_type=i, minimum_spots_cluster=minimum_spots_cluster)
            file_plots_cell_intensity_vs_num_spots = Plots().plot_cell_intensity_spots(dataframe,
                                                                                       number_of_spots_per_cell_nucleus,
                                                                                       number_of_spots_per_cell_cytosol,
                                                                                       output_identification_string,
                                                                                       spot_type=i)
            file_plots_spot_intensity_distributions = Plots().plot_spot_intensity_distributions(dataframe,
                                                                                                output_identification_string=output_identification_string,
                                                                                                remove_outliers=True,
                                                                                                spot_type=i)
            file_plots_distributions = Plots().plotting_results_as_distributions(number_of_spots_per_cell,
                                                                                 number_of_spots_per_cell_cytosol,
                                                                                 number_of_spots_per_cell_nucleus,
                                                                                 ts_size, number_of_TS_per_cell,
                                                                                 minimum_spots_cluster,
                                                                                 output_identification_string=output_identification_string,
                                                                                 spot_type=i)
            file_plots_cell_size_vs_num_spots = Plots().plot_cell_size_spots(channels_with_cytosol,
                                                                             channels_with_nucleus, cell_size,
                                                                             number_of_spots_per_cell, cyto_size,
                                                                             number_of_spots_per_cell_cytosol, nuc_size,
                                                                             number_of_spots_per_cell_nucleus,
                                                                             output_identification_string=output_identification_string,
                                                                             spot_type=i)
            ...
            list_files_distributions = [list_file_plots_spot_intensity_distributions, list_file_plots_distributions,
                                        list_file_plots_cell_size_vs_num_spots,
                                        list_file_plots_cell_intensity_vs_num_spots]
    ...
    def compare_intensities_spots_interpretation(self, merged_dataframe, list_dataframes, list_number_cells,
                                                 list_labels, plot_title_suffix, destination_folder, column_name,
                                                 remove_extreme_values=True, max_quantile=0.97, color_palete='CMRmap'):
        ...
        for i, column_with_intensity in enumerate(list_column_names):
            ...
            for j in range(len(list_dataframes)):
                list_cell_int.append(Utilities().function_get_df_columns_as_array(df=list_dataframes[j],
                                                                                  colum_to_extract=column_with_intensity,
                                                                                  extraction_type='values_per_cell'))
            df_cell_int = Utilities().convert_list_to_df(list_number_cells, list_cell_int, list_labels,
                                                         remove_extreme_values=remove_extreme_values,
            ...
            sns.set(font_scale=1.5)
            b = sns.scatterplot(data=data_frames, x=x_value_label, y=y_value_label, hue='variable', alpha=0.9,
        ...
        plt.savefig(file_name, transparent=False, dpi=360, bbox_inches='tight', format='pdf')
        plt.show()
        pathlib.Path().absolute().joinpath(file_name).rename(
            pathlib.Path().absolute().joinpath(destination_folder, file_name))

    def plot_single_cell_all_channels(self, image, df=None, spot_type=0, min_ts_size=4, show_spots=False,
                                      image_name=None, microns_per_pixel=None, max_percentile=99.8):
        # Extracting spot localization
        if not (df is None):
            y_spot_locations, x_spot_locations, y_TS_locations, x_TS_locations, number_spots, number_TS, number_spots_selected_z = Utilities().extract_spot_location_from_cell(
                df=df, spot_type=spot_type, min_ts_size=min_ts_size)
        ...
        for i in range(0, number_color_channels):
            ...
            if not (microns_per_pixel is None):
                scalebar = ScaleBar(dx=microns_per_pixel, units='um', length_fraction=0.25, location='lower right',
                                    box_color='k', color='w', font_properties=font_props)
    ...
    def plot_single_cell(self, image, df, selected_channel, spot_type=0, min_ts_size=4, show_spots=True,
                         image_name=None, microns_per_pixel=None, show_legend=True, max_percentile=99.5,
                         selected_colormap='plasma', show_title=True):
        # Extracting spot localization
        y_spot_locations, x_spot_locations, y_TS_locations, x_TS_locations, number_spots, number_TS, number_spots_selected_z = Utilities().extract_spot_location_from_cell(
            df=df, spot_type=spot_type, min_ts_size=min_ts_size)
            ...
            if show_spots == True:
                ...
                for i in range(number_spots):
                    if i < number_spots - 1:
                        ...
                        circle1 = plt.Circle((x_spot_locations[i], y_spot_locations[i]), 2, color='k', fill=False, lw=1,
                                             label='Spots = ' + str(number_spots))
                    axes[1].add_artist(circle1)
                    # Plotting TS
                if number_TS > 0:
                    for i in range(number_TS):
                        if i < number_TS - 1:
                            circleTS = plt.Circle((x_TS_locations[i], y_TS_locations[i]), 6, color='b', fill=False,
                                                  lw=3)
                        else:
                            circleTS = plt.Circle((x_TS_locations[i], y_TS_locations[i]), 6, color='b', fill=False,
                                                  lw=3, label='TS = ' + str(number_TS))
    ...
    def plot_selected_cell_colors(self, image, df, spot_type=0, min_ts_size=None, show_spots=True,
                                  use_gaussian_filter=True, image_name=None, microns_per_pixel=None, show_legend=True,
                                  list_channel_order_to_plot=[0, 1, 2], max_percentile=99.8):
        ...
        subsection_image_with_selected_cell_int8 = Utilities().convert_to_int8(max_subsection_image_with_selected_cell,
                                                                               rescale=True, min_percentile=0.5,
        ...
        while subsection_image_with_selected_cell_int8.shape[2] < 3:
            ...
            subsection_image_with_selected_cell_int8 = np.concatenate(
                (subsection_image_with_selected_cell_int8, zeros_plane[:, :, np.newaxis]), axis=2)
    ...
    def plot_complete_fov(self, list_images, df, number_of_selected_image, use_GaussianFilter=True,
                          microns_per_pixel=None, image_name=None, show_cell_ids=True, list_channel_order_to_plot=None,
                          min_percentile=10, max_percentile=99.5):
        ...
        max_complete_image_int8 = Utilities().convert_to_int8(max_complete_image, rescale=True,
        ...
        axes.set_yticks([])
        if not (microns_per_pixel is None):
            scalebar = ScaleBar(dx=microns_per_pixel, units='um', length_fraction=0.5, location='lower right',
                                box_color='k', color='w', font_properties=font_props)
        ...
        if show_cell_ids == True:
            ...
            for _, label in enumerate(cell_ids_labels):
                cell_idx_string = str(label)
                Y_cell_location = df_selected_cell.loc[df_selected_cell['cell_id'] == label, 'nuc_loc_y'].values[
                                      0] - moving_scale
    ...
    def plot_all_cells_and_spots(self, list_images, complete_dataframe, selected_channel,
                                 list_masks_complete_cells=[None], list_masks_nuclei=[None], spot_type=0,
                                 list_segmentation_successful=None, min_ts_size=4, image_name=None,
                                 microns_per_pixel=None, show_legend=True, show_plot=True, use_max_projection=True):
        ...
        for cell_id in range(0, number_cells):
            if NUM_ROWS == 1:
                ...
                axis_index = axes[r, c]
            image, df, cell_mask, nuc_mask, _ = Utilities().image_cell_selection(cell_id=cell_id,
        ...
        for cell_id in range(0, number_cells):
            ...
            if not (list_nuc_masks[0] is None) and not (list_cell_masks[0] is None):
                image, df, cell_mask, nuc_mask, _ = Utilities().image_cell_selection(cell_id=cell_id,
                                                                                     list_images=list_images,
                                                                                     mask_cell=list_cell_masks[cell_id],
                                                                                     mask_nuc=list_nuc_masks[cell_id],
                                                                                     dataframe=complete_dataframe)
            ...
            if use_max_projection == True:
                ...
                z_slice = central_z_slice
            y_spot_locations, x_spot_locations, y_TS_locations, x_TS_locations, number_spots, number_TS, number_spots_selected_z = Utilities().extract_spot_location_from_cell(
                df=df, spot_type=spot_type, min_ts_size=min_ts_size, z_slice=z_slice)
            max_visualization_value = np.percentile(temp_image, 99.5)
            min_visualization_value = np.percentile(temp_image, 0)
            # Plotting
            axis_index.imshow(temp_image, cmap='Greys', vmin=min_visualization_value, vmax=max_visualization_value)
            ...
            if not (cell_mask is None):
                ...
                axis_index.fill(downsampled_mask[:, 1], downsampled_mask[:, 0], facecolor='none', edgecolor='m',
                                linewidth=1.5)
            if not (nuc_mask is None):
                temp_contour = find_contours(nuc_mask, 0.5, fully_connected='high', positive_orientation='high')
                contour = np.asarray(temp_contour[0])
                downsampled_mask = signal.resample(contour, num=NUM_POINTS_MASK_EDGE_LINE)
                axis_index.fill(downsampled_mask[:, 1], downsampled_mask[:, 0], facecolor='none', edgecolor='b',
                                linewidth=1.5)
                # Plotting spots on image
            if number_spots_selected_z > 0:
                for i in range(number_spots_selected_z):
                    if i < number_spots_selected_z - 1:
                        circle1 = plt.Circle((x_spot_locations[i], y_spot_locations[i]), 2, color='r', fill=False,
                                             lw=0.3)
                    else:
                        circle1 = plt.Circle((x_spot_locations[i], y_spot_locations[i]), 2, color='r', fill=False,
                                             lw=0.3, label='Spots: ' + str(number_spots))
                    axis_index.add_artist(circle1)
                    # Plotting TS
            if number_TS > 0:
                for i in range(number_TS):
                    if i < number_TS - 1:
                        circleTS = plt.Circle((x_TS_locations[i], y_TS_locations[i]), 6, color='cyan', fill=False,
                                              lw=2.5)
                    else:
                        circleTS = plt.Circle((x_TS_locations[i], y_TS_locations[i]), 6, color='cyan', fill=False,
                                              lw=2.5, label='TS: ' + str(number_TS))
    ...
    def plot_all_cells(self, list_images, complete_dataframe, selected_channel, list_masks_complete_cells=[None],
                       list_masks_nuclei=[None], spot_type=0, list_segmentation_successful=None, min_ts_size=4,
                       show_spots=True, image_name=None, microns_per_pixel=None, show_legend=True, show_plot=True):
        # removing images where segmentation was not successful
        if not (list_segmentation_successful is None):
            list_images = [list_images[i] for i in range(len(list_images)) if list_segmentation_successful[i]]
        # Calculating number of subplots
                ...
                if number_spots > 0:
                    for i in range(number_spots):
                        if i < number_spots - 1:
                            ...
                            circle1 = plt.Circle((x_spot_locations[i], y_spot_locations[i]), 2, color='r', fill=False,
                                                 lw=0.5, label='Spots: ' + str(number_spots))
                        axis_index.add_artist(circle1)
                        # Plotting TS
                if number_TS > 0:
                    for i in range(number_TS):
...
class ColocalizationDistance():
    ...
    def __init__(self, df, list_spot_type_to_compare=[0, 1], time_point=0, threshold_intensity_0=0,
                 threshold_intensity_1=0, threshold_distance=2, show_plots=False, voxel_size_z=None, psf_z=None,
                 voxel_size_yx=None, psf_yx=None, report_codetected_spots_in_both_channels=False):
        self.df = df
    ...
    def extract_spot_classification_from_df(self):
        ...
        array_spot_type_per_cell = np.zeros((number_cells, 14)).astype(
            int)  # this array will store the spots separated  as types: spot_0_only, spot_1_only, or spot_0_1
        ...
        for cell_id in range(number_cells):
            ...
            array_spots_0 = np.asarray(self.df[['z', 'y', 'x']][(self.df["cell_id"] == cell_id) & (self.df[
                                                                                                       "spot_type"] == spot_type_0)])  # coordinates for spot_type_0 with shape [num_spots_type_0, 3]
            array_spots_1 = np.asarray(self.df[['z', 'y', 'x']][(self.df["cell_id"] == cell_id) & (self.df[
                                                                                                       "spot_type"] == spot_type_1)])  # coordinates for spot_type_1 with shape [num_spots_type_1, 3]
            ...
            distance_matrix = np.zeros((array_all_spots.shape[0], array_all_spots.shape[
                0]))  # the distance matrix is an square matrix resulting from the concatenation of both spot  types.
            for i in range(len(array_all_spots)):
                for j in range(len(array_all_spots)):
                    if j < i:
                        distance_matrix[i, j] = np.linalg.norm(
                            (array_all_spots[i, :] - array_all_spots[j, :]) * self.scale)
            ...
            column_with_cell_id = np.zeros(
                (coordinates_colocalized_spots.shape[0], 1)) + cell_id  # zeros column as 2D array
            coordinates_colocalized_spots = np.hstack(
                (coordinates_colocalized_spots, column_with_cell_id))  # append column
            ...
            is_spot_only_type_0 = np.all(negation_subsection_mask_distance_matrix,
                                         axis=0)  # Testing if all the columns are ones of inv(subsection_mask_distance_matrix). Representing spot type 0. Notice that np.all(arr, axis=0) does the calculation along the columns.
            localized_spots_in_spots_0_only = (
                        is_spot_only_type_0 > 0).nonzero()  # index_false_distance_matrix[:,1] # selecting the x-axis in [Y,X] matrix for 0_only spots
            coordinates_spots_0_only = array_spots_0[localized_spots_in_spots_0_only]
            column_with_cell_id_0_only = np.zeros(
                (coordinates_spots_0_only.shape[0], 1)) + cell_id  # zeros column as 2D array
            coordinates_spots_0_only = np.hstack(
                (coordinates_spots_0_only, column_with_cell_id_0_only))  # append column
            ...
            is_spot_only_type_1 = np.all(negation_subsection_mask_distance_matrix,
                                         axis=1)  # Testing if all the rows are ones of inv(subsection_mask_distance_matrix). Representing spot type 1. Notice that np.all(arr, axis=1) does the calculation along the rows.
            localized_spots_in_spots_1_only = (
                        is_spot_only_type_1 > 0).nonzero()  # index_false_distance_matrix[:,0] # selecting the y-axis in [Y,X] matrix for 1_only spots
            coordinates_spots_1_only = array_spots_1[localized_spots_in_spots_1_only]
            column_with_cell_id_1_only = np.zeros(
                (coordinates_spots_1_only.shape[0], 1)) + cell_id  # zeros column as 2D array
            coordinates_spots_1_only = np.hstack(
                (coordinates_spots_1_only, column_with_cell_id_1_only))  # append column
            ...
            if self.report_codetected_spots_in_both_channels == True:
                num_type_0_1 = (total_spots0 - num_type_0_only) + (
                            total_spots1 - num_type_1_only)  # Number of spots in both channels
                total_spots = num_type_0_only + num_type_1_only + num_type_0_1
            else:
                num_type_0_1 = coordinates_colocalized_spots.shape[
                    0]  # This will display the number of colocalized spots only in channel 0
                total_spots = num_type_0_only + num_type_1_only + num_type_0_1
            ...
            list_labels = ['time', 'ts_intensity_0', 'ts_intensity_1', 'ts_distance', 'image_id', 'cell_id',
                           'num_0_only', 'num_1_only', 'num_0_1', 'num_0', 'num_1', 'num_0_total', 'num_1_total',
                           'total']
            # creating a dataframe
        ...
        df_coordinates_colocalized_spots = pd.DataFrame(data=coordinates_colocalized_spots_all_cells,
                                                        columns=list_labels_coordinates)
        ...
        df_coordinates_0_only_spots = pd.DataFrame(data=coordinates_0_only_spots_all_cells,
                                                   columns=list_labels_coordinates)
        ...
        df_coordinates_1_only_spots = pd.DataFrame(data=coordinates_1_only_spots_all_cells,
                                                   columns=list_labels_coordinates)
...
class Metadata():
    ...
    def __init__(self, data_dir, channels_with_cytosol, channels_with_nucleus, channels_with_FISH, diameter_nucleus,
                 diameter_cytosol, minimum_spots_cluster, list_voxels=None, list_psfs=None, file_name_str=None,
                 list_segmentation_successful=True, list_counter_image_id=[], threshold_for_spot_detection=[],
                 number_of_images_to_process=None, remove_z_slices_borders=False, NUMBER_Z_SLICES_TO_TRIM=0,
                 CLUSTER_RADIUS=0, list_thresholds_spot_detection=[None], list_average_spots_per_cell=[None],
                 list_number_detected_cells=[None], list_is_image_sharp=[None], list_metric_sharpeness_images=[None],
                 remove_out_of_focus_images=False, sharpness_threshold=None):

        self.list_images, self.path_files, self.list_files_names, self.number_images = ReadImages(data_dir,
                                                                                                  number_of_images_to_process).read()
    ...
    def write_metadata(self):
        ...
        important_modules = ['tqdm', 'torch', 'tifffile', 'setuptools', 'scipy', 'scikit-learn', 'scikit-image',
                             'PyYAML', 'pysmb', 'pyfiglet', 'pip', 'Pillow', 'pandas', 'opencv-python-headless',
                             'numpy', 'numba', 'natsort', 'mrc', 'matplotlib', 'llvmlite', 'jupyter-core',
                             'jupyter-client', 'joblib', 'ipython', 'ipython-genutils', 'ipykernel', 'cellpose',
                             'big-fish']
            ...
            with open(filename, 'w') as fd:
                ...
                for indx, img_name in enumerate(self.list_files_names):
                    ...
                    if (self.list_segmentation_successful[indx] == True) and (self.list_is_image_sharp[indx] == True):
                        fd.write('\n        ' + img_name + (' ' * (difference_name_len + 4)) + ' ' * 8 + str(
                            self.list_metric_sharpeness_images[indx]) + '        ' + str(
                            self.list_counter_image_id[counter]))
                        ...
                        fd.write('\n        ' + img_name + (' ' * (difference_name_len + 4)) + ' ' * 8 + str(
                            self.list_metric_sharpeness_images[indx]) + '      - error out of focus.')
                        list_processing_image.append('error out of focus')
                        list_image_id.append(-1)
                    else:
                        fd.write('\n        ' + img_name + (' ' * (difference_name_len + 4)) + ' ' * 8 + str(
                            self.list_metric_sharpeness_images[indx]) + '      - error segmentation.')
...
class ReportPDF():
    ...
    def __init__(self, directory, filenames_for_pdf_report, channels_with_FISH, save_all_images,
                 list_z_slices_per_image, threshold_for_spot_detection, list_segmentation_successful=True):
    ...
    def create_report(self):
        ...
        for i, temp_file_name in enumerate(list_files_names):
            ...
            temp_original_img_name = pathlib.Path().absolute().joinpath(self.directory,
            ...
            if self.list_segmentation_successful[i] == True:
                temp_segmented_img_name = pathlib.Path().absolute().joinpath(self.directory,
            ...
            if (self.save_all_images == True) and (self.list_segmentation_successful[i] == True):
                for id_channel, channel in enumerate(self.channels_with_FISH):
                    ...
                    for z_slice in range(0, self.list_z_slices_per_image[i]):
                        temp_seg_name = pathlib.Path().absolute().joinpath(self.directory,
                                                                           'det_' + temp_file_name + '_ch_' + str(
                                                                               channel) + '_slice_' + str(
                                                                               z_slice) + '.png')
                        ...
                        if counter % 2 == 0:  # Adding space if is an even counter
                            ...
                            # Plotting the image
                            pdf.cell(w=0, h=0,
                                     txt='FISH Ch_ ' + str(channel) + '_slice_' + str(z_slice) + ': ' + temp_file_name,
                                     ln=2, align='L')
                    ...
                    try:
                        if (self.threshold_for_spot_detection[id_channel] is None):
                            temp_elbow_name = pathlib.Path().absolute().joinpath(self.directory,
                                                                                 'det_' + temp_file_name + '__elbow_' + '_ch_' + str(
                                                                                     channel) + '.png')
                            pdf.image(str(temp_elbow_name), x=0, y=HEIGHT // 2, w=WIDTH - 140)
                        else:
                            pdf.cell(w=0, h=10, txt='Used intensity threshold = ' + str(
                                self.threshold_for_spot_detection[id_channel]), ln=2, align='L')
                    except:
...
class SpotDetection():
    ...
    def __init__(self, image, FISH_channels, channels_with_cytosol, channels_with_nucleus, cluster_radius=500,
                 minimum_spots_cluster=4, masks_complete_cells=None, masks_nuclei=None, masks_cytosol_no_nuclei=None,
                 dataframe=None, image_counter=0, list_voxels=[[500, 160]], list_psfs=[[350, 160]], show_plots=True,
                 image_name=None, save_all_images=True, display_spots_on_multiple_z_planes=False,
                 use_log_filter_for_spot_detection=True, threshold_for_spot_detection=None, save_files=True):
        ...
        for i in range(0, len(self.list_FISH_channels)):
            ...
            [spotDetectionCSV, clusterDetectionCSV], image_filtered, threshold = BigFISH(self.image,
                                                                                         ...
                                                                                         self.threshold_for_spot_detection[
                                                                                             i],
                                                                                         save_files=self.save_files).detect()
...
class GaussianFilter():
    ...
    def apply_filter(self):
        ...
        for index_channels in range(0, number_channels):
            for index_time in range(0, number_time_points):
                if np.max(self.video[index_time, :, :, index_channels]) > 0:
                    video_bp_filtered_float[index_time, :, :, index_channels] = gaussian(
                        self.video[index_time, :, :, index_channels], self.sigma)
        ...
        for index_channels in range(0, number_channels):
            init_video = Parallel(n_jobs=self.NUMBER_OF_CORES)(
                delayed(img_uint)(video_bp_filtered_float[i, :, :, index_channels]) for i in
                range(0, number_time_points))
...
class NASConnection():
    ...
    def __init__(self, path_to_config_file, share_name='share'):
        ...
        self.conn = SMBConnection(username=usr, password=pwd, domain=domain, my_name=local_name,
                                  remote_name=str(self.server_name), is_direct_tcp=True)
    ...
    def copy_files(self, remote_folder_path, local_folder_path, timeout=600, file_extension=['.index', '.tif']):
        ...
        for file in list_dir:
            ...
            if (file.filename not in ['.', '..']) and any(file.filename.endswith(ext) for ext in file_extension):
                ...
                self.conn.retrieveFile(self.share_name, str(pathlib.Path(remote_folder_path).joinpath(file.filename)),
                                       fileobj)
                fileobj.close()
                # moving files in the local computer
                shutil.move(pathlib.Path().absolute().joinpath(file.filename),
                            local_folder_path.joinpath(file.filename))
    ...
    def write_files_to_NAS(self, local_file_to_send_to_NAS, remote_folder_path, timeout=600):
        ...
        if str(local_file_to_send_to_NAS.name) not in list_all_files_in_NAS:
            with open(str(local_file_to_send_to_NAS), 'rb') as file_obj:
                self.conn.storeFile(self.share_name,
                                    str(pathlib.Path(remote_folder_path).joinpath(local_file_to_send_to_NAS.name)),
                                    file_obj)
                print('The file was uploaded to NAS in location:',
                      str(pathlib.Path(remote_folder_path).joinpath(local_file_to_send_to_NAS.name)))
        return None
...
class ReadImages():
    ...
    def read(self):
        ...
        list_files_names_complete = sorted(
            [f for f in listdir(self.directory) if isfile(join(self.directory, f)) and ('.tif') in f],
            key=str.lower)  # reading all tif files in the folder
        list_files_names_complete.sort(key=lambda f: int(re.sub('\D', '', f)))  # sorting the index in numerical order
        path_files_complete = [str(self.directory.joinpath(f).resolve()) for f in
...
class MergeChannels():
    ...
    def checking_images(self):
        ...
        for _, _, files in os.walk(self.directory):
            for file in files:
                if ending_string.match(file) and file[
                    0] != '.':  # detecting a match in the end, not consider hidden files starting with '.'
                    prefix = file.rpartition('_')[
                        0]  # stores a string with the first part of the file name before the last underscore character in the file name string.
                    list_files_per_image = sorted(glob.glob(str(self.directory.joinpath(
                        prefix)) + '*.tif'))  # List of files that match the pattern 'file_prefix_C*.tif'
                ...
                if ending_string.match(file) and file[
                    ...
                    if len(list_files_per_image) > 1:  # creating merged files if more than one image with the exact ending substring is detected.
                        try:
                            ...
                            merged_img = np.concatenate([imread(list_files_per_image[i])[..., np.newaxis] for i, _ in
                                                         enumerate(list_files_per_image)], axis=-1).astype('uint16')
...
class Intensity():
    ...
    def __init__(self, original_image: np.ndarray, spot_size: int = 5, array_spot_location_z_y_x=[0, 0, 0],
                 method: str = 'disk_donut'):
    ...
    def calculate_intensity(self):
        ...
        def disk_donut(values_disk, values_donut, spot_size):
            ...
            mean_intensity_donut = np.mean(values_donut.flatten().astype('float'))  # mean calculation ignoring zeros
            spot_intensity_disk_donut = (mean_intensity_disk * (spot_size ** 2)) - (
                        mean_intensity_donut * (spot_size ** 2))
        ...
        for sp in range(self.number_spots):
            for i in range(0, self.number_channels):
                ...
                spots_range_to_replace = np.linspace(-(individual_spot_size - 1) / 2, (individual_spot_size - 1) / 2,
                                                     individual_spot_size, dtype=int)
                crop_range_to_replace = np.linspace(-(individual_spot_size + self.PIXELS_AROUND_SPOT - 1) / 2,
                                                    (individual_spot_size + self.PIXELS_AROUND_SPOT - 1) / 2,
                                                    individual_spot_size + self.PIXELS_AROUND_SPOT, dtype=int)

                crop_with_disk_and_donut = return_crop(self.original_image[z_pos, :, :, i], x_pos, y_pos,
                                                       spot_range=crop_range_to_replace)  #
                values_disk = return_crop(self.original_image[z_pos, :, :, i], x_pos, y_pos,
                                          spot_range=spots_range_to_replace)
                values_donut = return_donut(crop_with_disk_and_donut, spot_size=individual_spot_size)
                intensities_snr[sp, i], intensities_background_mean[sp, i], intensities_background_std[
                    sp, i] = signal_to_noise_ratio(values_disk, values_donut)  # SNR

                if self.method == 'disk_donut':
                    intensities_mean[sp, i], intensities_std[sp, i] = disk_donut(values_disk, values_donut,
                                                                                 spot_size=individual_spot_size)
                    ...
                    intensities_mean[sp, i], intensities_std[sp, i] = gaussian_fit(
                        values_disk)  # disk_donut(crop_image, self.disk_size
        return np.round(intensities_mean, 4), np.round(intensities_std, 4), np.round(intensities_snr, 4), np.round(
            intensities_background_mean, 4), np.round(intensities_background_std, 4)
...
class RemoveExtrema():
    ...
    def __init__(self, image: np.ndarray, min_percentile: float = 1, max_percentile: float = 99,
    ...
    def remove_outliers(self):
        ...
        if len(self.image.shape) == 2:
            ...
            if not np.max(
                    normalized_image_temp) == 0:  # this section detect that the channel is not empty to perform the normalization.
...
class Cellpose():
    ...
    def calculate_masks(self):
        ...
        if self.selection_method == None:
            selected_masks = model.eval(self.image, batch_size=self.BATCH_SIZE, normalize=True,
...
class BigFISH():
    ...
    def detect(self):
        ...
        if self.decompose_dense_regions == True:
            try:
                spots_post_decomposition, _, _ = detection.decompose_dense(image=rna,
                                                                           ...
                                                                           alpha=0.9,
                                                                           # alpha impacts the number of spots per candidate region
                                                                           beta=1,
                                                                           # beta impacts the number of candidate regions to decompose
        ...
        for i in range_plot_images:
            ...
            if not (self.image_name is None) and (i == central_slice) and (
                    self.show_plots == True):  # saving only the central slice
...
class DataProcessing():
    ...
    def __init__(self, spotDetectionCSV, clusterDetectionCSV, image, masks_complete_cells, masks_nuclei,
                 masks_cytosol_no_nuclei, channels_with_cytosol, channels_with_nucleus, yx_spot_size_in_px, spot_type=0,
                 dataframe=None, reset_cell_counter=False, image_counter=0, number_color_channels=None):
    ...
    def get_dataframe(self):
        ...
        def data_to_df(df, spotDetectionCSV, clusterDetectionCSV, mask_nuc=None, mask_cytosol_only=None,
                       masks_complete_cells=None, nuc_area=0, cyto_area=0, cell_area=0,
                       nuc_centroid_y=0, nuc_centroid_x=0, cyto_centroid_y=0, cyto_centroid_x=0, image_counter=0,
                       is_cell_in_border=0, spot_type=0, cell_counter=0,
                       nuc_int=None, cyto_int=None, complete_cell_int=None, pseudo_cyto_int=None,
                       nucleus_cytosol_intensity_ratio=None, nucleus_pseudo_cytosol_intensity_ratio=None):
            ...
            if not (self.channels_with_nucleus in (None, [None])):
                try:
                    ...
                    spots_no_ts, _, ts = multistack.remove_transcription_site(spotDetectionCSV, clusterDetectionCSV,
                                                                              mask_nuc, ndim=3)
                except:
            ...
            if not (self.channels_with_cytosol in (None, [None])) and not (
                ...
                try:
                    ...
                    spots_cytosol_only, _ = multistack.identify_objects_in_region(mask_cytosol_only,
                                                                                  spotDetectionCSV[:, :3], ndim=3)
                except:
                ...
                try:
                    ...
                    spots_cytosol_only, _ = multistack.identify_objects_in_region(masks_complete_cells,
                                                                                  spotDetectionCSV[:, :3], ndim=3)
                except:
                    spots_cytosol_only = None
            else:
            ...
            for c in range(self.number_color_channels):
                ...
                if not (self.channels_with_cytosol in (None, [None])):
                    array_complete[:, self.NUMBER_OF_CONSTANT_COLUMNS_IN_DATAFRAME + self.number_color_channels + c] = \
                    cyto_int[c]
                if not (self.channels_with_cytosol in (None, [None])):
                    # populating with complete_cell_int
                    array_complete[:,
                    self.NUMBER_OF_CONSTANT_COLUMNS_IN_DATAFRAME + self.number_color_channels * 2 + c] = \
                    complete_cell_int[c]
                    # populating with pseudo_cyto_int
                array_complete[:, self.NUMBER_OF_CONSTANT_COLUMNS_IN_DATAFRAME + self.number_color_channels * 3 + c] = \
                pseudo_cyto_int[c]
                # populating with nucleus_cytosol_intensity_ratio
                array_complete[:, self.NUMBER_OF_CONSTANT_COLUMNS_IN_DATAFRAME + self.number_color_channels * 4 + c] = \
                nucleus_cytosol_intensity_ratio[c]
                # populating with nucleus_pseudo_cytosol_intensity_ratio
                array_complete[:, self.NUMBER_OF_CONSTANT_COLUMNS_IN_DATAFRAME + self.number_color_channels * 5 + c] = \
                nucleus_pseudo_cytosol_intensity_ratio[c]
            ...
            if num_ts > 0:
                ...
                intensity_ts = \
                Intensity(original_image=self.image, spot_size=cluster_spot_size, array_spot_location_z_y_x=ts[:, 0:3],
                          method='disk_donut').calculate_intensity()[0]
            if num_nuc > 0:
                intensity_spots_nuc = Intensity(original_image=self.image, spot_size=self.yx_spot_size_in_px,
                                                array_spot_location_z_y_x=spots_nuc[:, 0:3],
                                                method='disk_donut').calculate_intensity()[0]
            if num_cyto > 0:
                intensity_spots_cyto = Intensity(original_image=self.image, spot_size=self.yx_spot_size_in_px,
                                                 array_spot_location_z_y_x=spots_cytosol_only[:, 0:3],
                                                 method='disk_donut').calculate_intensity()[0]
            ...
            number_columns_after_adding_intensity_in_cell = self.NUMBER_OF_CONSTANT_COLUMNS_IN_DATAFRAME + (
                        6 * self.number_color_channels)
            array_complete[:,
            number_columns_after_adding_intensity_in_cell: number_columns_after_adding_intensity_in_cell + self.number_color_channels] = array_spot_int
            ...
            new_dtypes = {'image_id': int, 'cell_id': int, 'spot_id': int, 'is_nuc': int, 'is_cluster': int,
                          'nuc_loc_y': int, 'nuc_loc_x': int, 'cyto_loc_y': int, 'cyto_loc_x': int, 'nuc_area_px': int,
                          'cyto_area_px': int, 'cell_area_px': int, 'x': int, 'y': int, 'z': int, 'cluster_size': int,
                          'spot_type': int, 'is_cell_fragmented': int}
        ...
        if (not (self.dataframe is None)) and (
                self.reset_cell_counter == False):  # IF the dataframe exist and not reset for multi-channel fish is passed
            new_dataframe = self.dataframe
            counter_total_cells = np.max(self.dataframe['cell_id'].values) + 1

        elif (not (self.dataframe is None)) and (
                self.reset_cell_counter == True):  # IF dataframe exist and reset is passed
            new_dataframe = self.dataframe
            counter_total_cells = np.max(
                self.dataframe['cell_id'].values) - n_masks + 1  # restarting the counter for the number of cells
            ...
            new_dataframe = pd.DataFrame(
                columns=['image_id', 'cell_id', 'spot_id', 'nuc_loc_y', 'nuc_loc_x', 'cyto_loc_y', 'cyto_loc_x',
                         'nuc_area_px', 'cyto_area_px', 'cell_area_px', 'z', 'y', 'x', 'is_nuc', 'is_cluster',
                         'cluster_size', 'spot_type',
                         'is_cell_fragmented'] + list_columns_intensity_nuc + list_columns_intensity_cyto + list_columns_intensity_complete_cell + list_columns_intensity_pseudo_cyto + list_nucleus_cytosol_intensity_ratio + list_nucleus_pseudo_cytosol_intensity_ratio + list_intensity_spots + list_intensity_clusters)
            counter_total_cells = 0
        # loop for each cell in image

        num_pixels_to_dilate = 30
        for id_cell in range(0,
                             n_masks):  # iterating for each mask in a given cell. The mask has values from 0 for background, to int n, where n is the number of detected masks.
            # calculating nuclear area and center of mass
            if not (self.channels_with_nucleus in (None, [None])):
                nuc_area, nuc_centroid_y, nuc_centroid_x = mask_selector(self.masks_nuclei[id_cell],
                ...
                for k in range(self.number_color_channels):
                    ...
                    pseudo_cyto_int[k] = np.round(temp_masked_img_with_pseudo_cytosol_mask[
                                                      np.nonzero(temp_masked_img_with_pseudo_cytosol_mask)].mean(), 5)
                    # if k ==0:
            ...
            if not (self.channels_with_cytosol in (None, [None])):
                cell_area, cyto_centroid_y, cyto_centroid_x = mask_selector(self.masks_complete_cells[id_cell],
                                                                            calculate_centroid=True)
            ...
            # case where cyto is passed but not nucleus
                ...
                slected_masks_cytosol_no_nuclei, _, _ = mask_selector(self.masks_complete_cells[id_cell],
                                                                      calculate_centroid=False)
                cyto_area, _, _ = mask_selector(self.masks_complete_cells[id_cell],
                                                calculate_centroid=False)  # if not nucleus channel is passed the cytosol is consider the complete cell.
                selected_masks_complete_cells = self.masks_complete_cells[id_cell]
            else:
                slected_masks_cytosol_no_nuclei = None
                cyto_area = 0
            ...
            is_cell_in_border = np.any(np.concatenate((tested_mask_for_border[:, 0], tested_mask_for_border[:, -1],
                                                       tested_mask_for_border[0, :], tested_mask_for_border[-1, :])))
            # Data extraction
